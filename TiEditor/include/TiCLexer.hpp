// This defines the interface to the TiCLexer class.
//
...
//! \brief The TiCLexer class encapsulates the Scintilla C++
//! lexer with the Advanced TiBasic lexer.
class TiCLexer : public QsciLexer
{
    Q_OBJECT
...
public:
    ...
    //! Construct a TiCLexer with parent \a parent.  \a parent is typically
    //! the QsciScintilla instance.  \a caseInsensitiveKeywords is true if the
    //! lexer ignores the case of keywords.
    TiCLexer(QObject *parent = 0, bool caseInsensitiveKeywords = false);

    //! Destroys the TiCLexer instance.
    virtual ~TiCLexer();
...
private:
    ...
    TiCLexer(const TiCLexer &);
    TiCLexer &operator=(const TiCLexer &);
    
    std::string m_tibasicKeywords;
};
