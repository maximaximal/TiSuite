...
int Unsafe::checkStringVariableType(const std::string &str)
{
    for(std::size_t i = 0; i < 199; ++i) 
    {
        if(StandardTokens[i].text == str) {
            return 1;
        }
    }
    for(std::size_t i = 0; i < 302; ++i) 
...
std::string Unsafe::generateTiTokensList()
{
    std::string tokens;
    for(std::size_t i = 0; i < 199; ++i) 
    {
        tokens.append(StandardTokens[i].text);
        tokens.append(" ");
    }
    return tokens;
}
std::string Unsafe::generateTiVarsList()
{
    std::string tokens;
    for(std::size_t i = 0; i < 199; ++i) 
    {
        tokens.append(CalcVars[i].text);
        tokens.append(" ");
    }
    return tokens;
}
